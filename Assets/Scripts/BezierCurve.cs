...
public class BezierCurve : MonoBehaviour
{
    ...
    public void SetRoute(Vector2 _node0Pos, Vector2 _node1Pos, Vector2 _nodeNPos)
    {      
		nodes = new Vector3[5];
		for(int i = 0; i < nodes.Length; i++)
		{
			nodes[i] = Vector3.zero;
		}
		nodes[0] = new Vector3(_node0Pos.x, 10, _node0Pos.y); 
		nodes[1] =  new Vector3(_node1Pos.x, 10, _node1Pos.y);
		nodes[2] =  new Vector3(_nodeNPos.x, 10, _nodeNPos.y);
